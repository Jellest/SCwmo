multiple_import_ahn <- function(aws_list, sensor_name, LONLAT = FALSE, radius, resolution = 0.5, AHN3= FALSE, raw_ahn = TRUE, terrain_ahn = TRUE, delete_sheets = TRUE, redownload_ahn = FALSE){
  start_time <- Sys.time()
  for (a in 1:length(aws_list)){
    ...
    import_single_ahn(aws_name = aws_list[a],
