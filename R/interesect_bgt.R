...
clip_bgt <- function(aws_name, coords, bgt_shape, temperature_criteria.df, class, go_to_next_class, cv_colName){
  ...
  ##load criteria
  tluc <- filter(guideline_criteria, Variable == "temperature" & Criteria_Type == "land use" & Class == class)
  ...
  df <- data.frame(outer_objectCount = numeric(nrow(artificial_objects.sf)),
                   ...
                   outer_sumAreas = numeric(nrow(artificial_objects.sf)),
                   ...
                   inner_sumAreas = numeric(nrow(artificial_objects.sf)),
  ...
  if(class >= 5){
    ...
    print(outer_relAreaColName)
    View(temperature_criteria.df[,outer_relAreaColName])
