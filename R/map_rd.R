library(leaflet)
library(mapview)
map_rd <- function(aws_name, sensor_name, addition = "", buffers, vegetation_height_raster, vegetation_radius, AHN3 = FALSE, aws.df = AWS.df, class = ""){
...
#     
  ...
  if(vegetation == TRUE){
    map <- addLegend(map, position = "topright", values = values(vegetation_height_raster), pal = pal,
                title = "Vegetation height (m)", group = "Vegetation height")
  }
