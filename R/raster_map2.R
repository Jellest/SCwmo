...
raster_map <- function(aws_name, sensor_name, addition = "", diff_raster, shadow_raster ,aws.df = AWS.df){
  ...
  map <- addLegend(map, position = "topright", values = aws_name, color = "#e9f", labels = aws_name,
...
raster_map(aws_name = "Vlissingen",
           ...
           diff_raster = raster("data/AHN3/Vlissingen/raw/Vlissingen_aws_AHN3_raw_ahn.tif"),
           shadow_raster = raster("output/Vlissingen/solar_shadow_angles/rasters/AHN3/Shadows/Vlissingen_AHN3_sa_171.526696898183.tif")
)
