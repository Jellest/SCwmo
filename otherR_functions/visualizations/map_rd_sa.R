...
#     
  ...
  if(missing(vegetation_height_raster)){
    ...
    path <- paste0("output/", aws_name_trim, "/solar_shadow_angles/rasters/", AHN, "/Shadows/DeBilt_AHN3_sa_99.7438895423655.tif")
    print(path)
    shadow_angles_raster <- raster(path)
  }
  #vegetation <- FALSE
  ...
  if(vegetation == TRUE){
    overlays <- append(overlays, "Shadow angles")
    pal <- colorNumeric(c("#fafa00", "#000000"), values(shadow_angles_raster), na.color = "transparent")
    map <- addRasterImage(map, shadow_angles_raster, colors = pal, opacity = 1.0, group = "Shadow angles")
  }
  if(circles == TRUE){
    #add circles
  ...
  map <-     addCircleMarkers(map, data = single_aws[1,], lng = single_aws[1,"LON"], lat = single_aws[1,"LAT"],
                              radius = 3,  stroke = TRUE, color = "red", weight = 1.0,
                              fillColor = "red", fillOpacity = 1.0,
                              group = "Temperature sensor"
  ) %>%
    addCircleMarkers(lng = 5.17969000, lat = 52.09883000,
                     radius = 3,  stroke = TRUE, color = "#45f215", weight = 1.0,
                     fillColor = "#45f215", fillOpacity = 1.0,
                     group = "Anemometer"
    )
  #add legend and control
  map <- addLegend(map, position = "topright", values = aws_name, color = "red", labels = aws_label,
              title = " Air temperature sensor", group = "AWS")
  map <- addLegend(map, position = "topright", values = aws_name, color = "#45f215", labels = aws_label,
                   title = "Wind mast", group = "Anemometer")
  if(circles == TRUE){
  ...
  if(vegetation == TRUE){
    map <- addLegend(map, position = "topright", values = values(shadow_angles_raster), pal = pal,
                title = "Shadow angle (degrees)", group = "Shadow angles")
  }
...
shadow_angle_map <- map_rd(aws_name = "De Bilt",
...
mapshot(shadow_angle_map, file = "shadow_angles_map.png", url = NULL, remove_url = TRUE, remove_controls = c("zoomControl", "layersControl", "homeButton"))
veg_map <- function(){
  ras <- raster(paste0("output/DeBilt/vegetation_height/DeBilt_AHN3_10m_height_difference.tif"))
  breaks <- c(0,0.1,0.4,0.6)
  pal <- colorNumeric(c("#99d8c9", "#2ca25f"), values(ras), na.color = "transparent")

 raster::plot(ras)
 
 #ggsave(filename = "C:/Users/3691233/Dropbox/Apps/ShareLaTeX/Automated Temperature SC  MSc Thesis/Figures/Results/vegetation_height.png", plot = last_plot())
}
